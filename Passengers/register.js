...
function validateForm() {
  ...
  const dup = passengers.find(
    (p) =>
      p.passport.toLowerCase() === fields.passport.value.trim().toLowerCase() &&
      p.id !== currentId,
  );
  if (dup) {
...
function renderTable(query = "") {
  ...
  if (filtered.length === 0) {
    ...
    filtered.forEach((p, index) => {
      ...
      row.innerHTML = `
        <td class="radio-col">
          <input type="radio" name="selectPassenger" class="radio-select" data-id="${p.id}">
        </td>
        <td>${index + 1}</td>
  ...
  if (savedId) {
    const radio = document.querySelector(
      `input.radio-select[data-id="${savedId}"]`,
    );
    if (radio) {
      radio.checked = true;
...
function updateStats(passengers) {
  ...
  document.getElementById("stat-male").textContent = passengers.filter(
    (p) => p.gender === "Male",
  ).length;
  document.getElementById("stat-female").textContent = passengers.filter(
    (p) => p.gender === "Female",
  ).length;
}
